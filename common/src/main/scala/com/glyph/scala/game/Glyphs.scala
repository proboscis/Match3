...
import com.glyph.scala.lib.libgdx.GdxOps
import com.glyph.scala.lib.util.GlyphLibOps
...
object Glyphs
  ...
  implicit class TryOps[T](val t:Try[T]) extends AnyVal{
    def toVnel:ValidationNel[Throwable,T] = t match {
      case util.Success(s) => s.successNel
      case util.Failure(f) => f.failNel
    }
  }
  implicit class FutureOps[T](val f:Future[T]) extends AnyVal{
    def valueVnel:ValidationNel[Throwable,T] = f.value match {
